...
export function registerProductRoutes(app: any, ctx: (c: any) => any, body: (c: any) => Promise<any>) {
  ...
  app.get('/projects/:projectId/keywords/search', async (c: any) => c.json(await workspaceCommands.keywordSearch(ctx(c), {
    ...
    q: c.req.query('q'),
    candidateStatus: c.req.query('candidateStatus'),
    clusterId: c.req.query('clusterId'),
    existingPage: c.req.query('existingPage'),
    researchStatus: c.req.query('researchStatus'),
    provider: c.req.query('provider'),
    sort: c.req.query('sort'),
    limit: Number(c.req.query('limit') ?? 50),
    offset: Number(c.req.query('offset') ?? 0)
  })));
  app.get('/projects/:projectId/keywords/:keywordId/detail', async (c: any) => c.json(await workspaceCommands.keywordDetail(ctx(c), { projectId: c.req.param('projectId'), keywordId: c.req.param('keywordId') })));
  ...
  app.post('/projects/:projectId/discovery-jobs/:jobId/claim', async (c: any) => c.json(await discoveryCommands.claim(ctx(c), { ...(await body(c)), projectId: c.req.param('projectId'), jobId: c.req.param('jobId') })));
  app.post('/projects/:projectId/discovery-jobs/:jobId/heartbeat', async (c: any) => c.json(await discoveryCommands.heartbeat(ctx(c), { ...(await body(c)), projectId: c.req.param('projectId'), jobId: c.req.param('jobId') })));
  app.post('/projects/:projectId/discovery-jobs/:jobId/recover', async (c: any) => c.json(await discoveryCommands.recoverExpired(ctx(c), { projectId: c.req.param('projectId'), jobId: c.req.param('jobId') })));
  ...
  app.post('/projects/:projectId/discovery-candidates/bulk-review', async (c: any) => c.json(await discoveryCommands.bulkReviewCandidates(ctx(c), { ...(await body(c)), projectId: c.req.param('projectId') })));
