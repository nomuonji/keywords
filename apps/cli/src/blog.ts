...
export function registerBlogCli(program:any,base:CommandContext){
 program.command('blog').argument('<operation>','context | importContext | prepare | export | get | receipt | capture | evaluate | expand | observations')
 ...
 .action(async(operation:string,projectId:string,opts:any)=>{
   ...
   if(typeof command!=='function'||!['contract','context','importContext','prepare','export','get','receipt','capture','evaluate','expand','observations','observe'].includes(operation))throw new Error('Unknown Blog operation');
