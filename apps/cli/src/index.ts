...
research.command('serp').argument('<projectId>').argument('<query>').option('--country <country>').option('--language <language>').option('--location <location>').option('--num <number>').option('--provider <provider>', 'brave (default) or serper').action(async (projectId: string, query: string, opts: {country?: string; language?: string; location?: string; num?: string; provider?: 'brave'|'serper'}) => print(await commands.research.serp(ctx, { projectId, query, country: opts.country, language: opts.language, location: opts.location, num: opts.num ? Number(opts.num) : undefined, provider: opts.provider })));
