...
type View='portfolio'|'home'|'discovery'|'keywords'|'planning'|'performance'|'work'|'settings';
...
export function App(){
 const [projects,setProjects]=useState<Project[]>([]),[projectId,setProjectId]=useState(''),[view,setView]=useState<View>('portfolio'),[snapshot,setSnapshot]=useState<Snapshot|null>(null),[tasks,setTasks]=useState<Task[]>([]),[error,setError]=useState(''),[busy,setBusy]=useState(''),[loading,setLoading]=useState(false),[refreshKey,setRefreshKey]=useState(0);
