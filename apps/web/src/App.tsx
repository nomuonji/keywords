...
type View='articles'|'portfolio'|'autopilot'|'home'|'discovery'|'keywords'|'planning'|'performance'|'work'|'settings';
...
export function App(){
 const [projects,setProjects]=useState<Project[]>([]),[projectId,setProjectId]=useState(''),[view,setView]=useState<View>('articles'),[snapshot,setSnapshot]=useState<Snapshot|null>(null),[error,setError]=useState(''),[busy,setBusy]=useState(''),[loading,setLoading]=useState(false),[refreshKey,setRefreshKey]=useState(0);
 ...
 useEffect(()=>{abortRef.current?.abort();setSnapshot(null);if(projectId){const nextView=nextProjectViewRef.current;nextProjectViewRef.current=null;setView(current=>nextView??(['articles','portfolio','autopilot'].includes(current)?current:'home'));void loadHome(projectId)}return()=>abortRef.current?.abort()},[projectId]);
 ...
 const globalView=['articles','portfolio','autopilot'].includes(view);
 return <div className="app productApp"><aside><div className="brand"><span className="dot"/>KEYWORDS</div><button className={view==='articles'?'project active':'project'} onClick={()=>setView('articles')}><b>記事</b><small>完成・制作中・要判断</small></button><button className={view==='portfolio'?'project active':'project'} onClick={()=>setView('portfolio')}>サイト群の実績</button><button className={view==='autopilot'?'project active':'project'} onClick={()=>setView('autopilot')}><b>実行詳細</b><small>Autopilot・内部状態</small></button><div className="sideSection"><h4>Projects</h4><div className="projectList">{projects.map(p=><button className={p.id===projectId?'project active':'project'} onClick={()=>{if(p.id===projectId)setView('home');else{nextProjectViewRef.current='home';setProjectId(p.id)}}} key={p.id}><b>{p.name}</b><small>{p.domain||'topic only'}</small></button>)}</div><form className="stack createProject" onSubmit={createProject}><input name="name" placeholder="新しいプロジェクト" required/><input name="domain" placeholder="domain（任意）"/><button disabled={busy==='project'}>{busy==='project'?'作成中…':'作成'}</button></form></div>{projectId&&<nav className="productNav">{nav.map(item=><button key={item.id} className={view===item.id?'active':''} onClick={()=>setView(item.id)}><b>{item.label}</b><small>{item.hint}</small></button>)}</nav>}<div className="sideFooter"><small>Headless-first · Human-governed</small><small>Local completion ≠ publication</small></div></aside>
 <main className={globalView?'portfolioMain':undefined}>{error&&!globalView&&<div className="error globalError" role="alert">{error}<button aria-label="エラーを閉じる" onClick={()=>setError('')}>×</button></div>}{view==='articles'?<ArticlesOverview/>:view==='portfolio'?<PortfolioWorkspace onOpen={(id,targetView)=>{if(id===projectId)setView(targetView);else{nextProjectViewRef.current=targetView;setProjectId(id)}}}/>:view==='autopilot'?<AutopilotOverview/>:!projectId?<section className="empty"><h1>プロジェクトを作成</h1><p>サイトがなくてもテーマだけで探索を開始できます。</p></section>:loading&&!snapshot?<section className="empty"><h1>ワークスペースを読み込み中…</h1></section>:snapshot?<><header className="productHeader"><div><p className="eyebrow">SEO OPERATIONS</p><h1>{snapshot.project.name}</h1><p>{snapshot.project.domain||'テーマ起点のプロジェクト'}</p></div><div className="agent"><span className="pulse"/>共有状態 接続中</div></header>
