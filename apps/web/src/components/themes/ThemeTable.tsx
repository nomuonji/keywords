  ...
  if (!themes.length) {
    return (
      <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">
        自動対象のテーマがありません。プロジェクト設定から autoUpdate を有効化してください。
      </div>
    );
  }
        ...
        return (
          ...
          >
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 ${
                      theme.pendingNodes > 0
                        ? 'bg-warning/10 text-warning'
                        : 'bg-slate-200 text-slate-600'
                    }`}
                  >
                    <MdHourglassEmpty size={14} />
                    {theme.pendingNodes} 件待ち
                  </span>
                </div>
              </div>
            </button>
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                className="inline-flex items-center gap-1 rounded-md bg-primary px-2 py-1 text-xs font-medium text-white shadow hover:bg-primary/90"
