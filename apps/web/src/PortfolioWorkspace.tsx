...
export function PortfolioWorkspace({ onOpen }: { onOpen: (id: string, view: 'performance' | 'work') => void }) {
  const [data, setData] = useState<Portfolio | null>(null), [error, setError] = useState(''), [filter, setFilter] = useState(''), [revision, setRevision] = useState(0), [loading, setLoading] = useState(false);
  useEffect(() => { const abort = new AbortController(); setError(''); setLoading(true); api<Portfolio>('/portfolio', { signal: abort.signal }).then(setData).catch(e => { if (!abort.signal.aborted) setError(String(e)); }).finally(() => setLoading(false)); return () => abort.abort(); }, [revision]);
  const sites = data?.sites.filter(site => `${site.name} ${site.host} ${site.signal}`.toLowerCase().includes(filter.toLowerCase())) ?? [];
  return <div className="productStack portfolioWorkspace"><header className="productHeader"><div><p className="eyebrow">BLOG PORTFOLIO</p><h1>サイト群の実績</h1><p>GA4・検索流入から確認するサイトを選び、改善作業と公開後の観測へ進みます。</p></div><button onClick={() => setRevision(v => v + 1)} disabled={loading}>{loading ? '読み込み中…' : '保存データを再読込'}</button></header>
    {error && <div className="error" role="alert">{error}</div>}
    {!data && !error && <p className="loadingNote" role="status">実績を読み込み中…</p>}
    {data && <><section className="panel"><p>更新日時: {data.generatedAt ? new Date(data.generatedAt).toLocaleString('ja-JP') : '不明'} · {data.sites.length}サイト</p>
      ...
      {data.message && <p className="inlineNotice" role="alert">{data.message}</p>}
    ...
    </section><section className="panel portfolioTablePanel" style={{ overflowX: 'auto' }}><table><caption className="srOnly">サイト別の検索・アクセス実績</caption><thead><tr><th>サイト / 確認事項</th><th>検索クリック</th><th>検索表示</th><th>GA4セッション</th><th>作業</th><th>Keywordsで開く</th></tr></thead><tbody>
