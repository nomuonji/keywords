...
const coreTools = [
  ...
  { name: 'research_serp', description: 'Fetch normalized SERP evidence; Brave is default and Serper is opt-in.', inputSchema: s('SERP', { projectId: { type: 'string' }, query: { type: 'string' }, country: { type: 'string' }, language: { type: 'string' }, location: { type: 'string' }, num: { type: 'number' }, provider: { type: 'string', enum: ['brave','serper'] } }, ['projectId','query']) },
