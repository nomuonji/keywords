...
const tools = [
  ...
  { name: 'research_serp', description: 'Query and persist a normalized SERP. Brave is the default provider; Serper is opt-in.', inputSchema: s('SERP research', { projectId: { type: 'string' }, query: { type: 'string' }, country: { type: 'string' }, language: { type: 'string' }, location: { type: 'string' }, num: { type: 'number' }, provider: { type: 'string', enum: ['brave','serper'] } }, ['projectId','query']) },
