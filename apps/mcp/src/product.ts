...
export const productTools = [
  ...
  { name: 'operation_discovery_start', description: 'Start and atomically claim normal keyword discovery from an active operation under delegated limits.', inputSchema: s('Operation discovery', { operationId: { type: 'string' }, projectId: { type: 'string' }, seedKeywords: { type: 'array', items: { type: 'string' } }, targetUrl: { type: 'string' }, goal: { type: 'string' }, language: { type: 'string' }, country: { type: 'string' }, region: { type: 'string' }, excludedTerms: { type: 'array', items: { type: 'string' } }, maxCandidates: { type: 'number' }, maxExternalRequests: { type: 'number' }, leaseSeconds: { type: 'number' } }, ['operationId','projectId','goal']) },
