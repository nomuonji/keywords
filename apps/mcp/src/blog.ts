...
const definitions:Array<[string,string,Record<string,unknown>,string[]]>=[
 ...
 ['writeDraft','Atomically write the real article file under KEYWORDS_BLOG_ROOT and persist an artifact manifest. Use this after blog_prepare; a plan alone is not article completion.',{operationId:string,pageId:string,content:string,artifactPath:string,articleId:string,sourceIds:stringArray,buildCommand:string},['operationId','pageId','content']],
