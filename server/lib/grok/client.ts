...
export class GrokClient {
  ...
  async summarize(params: {
    ...
    if (!this.client) {
      return Promise.resolve({ ...params.group, outlineTitle: '(Grok disabled) Dummy Outline', h2: ['Dummy H2'] });
    }
  ...
  async generateArticle(params: GenerateArticleParams): Promise<GenerateArticleResult> {
    if (!this.client) {
      return Promise.resolve({ title: '(Grok disabled) Dummy Title', html: '<p>This is a dummy article content.</p>' });
    }
  ...
  async summarizeCluster(input: SummarizeClusterInput): Promise<SummarizeClusterOutput> {
    if (!this.client) {
      return Promise.resolve({ id: input.groupId, title: input.representativeKw, outlineTitle: '(Grok disabled) Dummy Outline', h2: ['Dummy H2'], intent: 'info' });
    }
  ...
  async suggestThemes(input: SuggestThemesInput): Promise<SuggestThemesOutput> {
    if (!this.client) {
      return Promise.resolve(['(Grok disabled) Dummy Theme 1', '(Grok disabled) Dummy Theme 2']);
    }
  ...
  async suggestNodes(input: SuggestNodesInput): Promise<SuggestNodesOutput> {
    if (!this.client) {
      return Promise.resolve(['(Grok disabled) Dummy Topic 1', '(Grok disabled) Dummy Topic 2']);
    }
  ...
  async clusterKeywords(input: ClusterKeywordsInput): Promise<ClusterKeywordsOutput> {
    if (!this.client) {
      return Promise.resolve([{ keywords: input.keywords }]);
    }
  ...
  private parseSuggestions(text: string): string[] {
    try {
      ...
      if (parsed && Array.isArray(parsed.suggestions)) {
        return parsed.suggestions.filter((item: unknown): item is string => typeof item === 'string' && item.trim().length > 0);
      }
      return [];
    } catch (error) {
  ...
  private buildOutlinePrompt(input: SummarizeClusterInput): string {
    const keywordList = input.keywords
      .map((kw: KeywordDocWithId) => `- ${kw.text} (volume: ${kw.metrics.avgMonthly ?? 'n/a'})`)
      .join('\n');
    return [
  ...
  private buildClusterKeywordsPrompt(input: ClusterKeywordsInput): string {
    const keywordList = input.keywords.map((kw: { id: string; text: string }) => `{"id": "${kw.id}", "text": "${kw.text}"}`).join(',\n');
    return [
      'You are an expert SEO content strategist.',
