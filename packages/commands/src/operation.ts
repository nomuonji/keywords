...
function operationRow(id: string) {
  return one('SELECT * FROM operation_requests WHERE id=?', id);
}
...
function operationView(id: string) {
  ...
  const projects = rows(`SELECT op.*,p.name,p.domain,ws.objective AS work_objective,ws.status AS work_status,ws.summary AS work_summary,ws.last_next_action,
    ...
    WHERE op.operation_id=? ORDER BY p.name`, id).map(row => ({
      projectId: row.project_id, name: row.name, domain: row.domain, status: row.status, blocker: row.blocker,
...
function emitEvent(input: { operationId?: string | null; projectId?: string | null; kind: string; severity?: 'info' | 'warning' | 'error'; key: string; payload: unknown }) {
  const existing = one('SELECT id FROM operation_events WHERE dedupe_key=?', input.key);
  if (existing) return existing.id as string;
  const eventId = randomUUID();
  run('INSERT INTO operation_events(id,operation_id,project_id,kind,severity,dedupe_key,payload_json,created_at) VALUES(?,?,?,?,?,?,?,?)',
    eventId, input.operationId ?? null, input.projectId ?? null, input.kind, input.severity ?? 'info', input.key, JSON.stringify(input.payload ?? null), now());
  return eventId;
}
...
function createChildWork(ctx: CommandContext, operationId: string, projectId: string, objective: string, criteria: string[], maxActions: number) {
  ...
  if (existing) {
    ...
    childStatus = existing.status === 'running' ? 'running' : existing.status;
  ...
  if (!existingTask) {
    const t = now();
    run('INSERT INTO tasks(id,project_id,title,description,status,priority,assignee_type,related_type,related_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
      taskId, projectId, objective.slice(0, 240), `Agent-driven operation ${operationId}`, 'doing', 90, 'agent', 'operation', operationId, t, t);
  }
  run(`INSERT INTO operation_projects(operation_id,project_id,work_session_id,task_id,status,blocker,updated_at) VALUES(?,?,?,?,?,?,?)
    ON CONFLICT(operation_id,project_id) DO UPDATE SET work_session_id=excluded.work_session_id,task_id=excluded.task_id,status=excluded.status,blocker=excluded.blocker,updated_at=excluded.updated_at`,
    operationId, projectId, sessionId, taskId, childStatus, blocker, now());
  return { projectId, workSessionId: sessionId, taskId, status: childStatus, reusedSession: Boolean(existing), blocker };
}
...
function executorView(row: any) {
  if (!row) return null;
  return { id: row.id, kind: row.kind, status: row.status, capabilities: parse(row.capabilities_json, []), currentOperationId: row.current_operation_id, currentProjectId: row.current_project_id, generation: row.generation, lastSeenAt: row.last_seen_at, leaseExpiresAt: row.lease_expires_at, connected: Boolean(row.last_seen_at && Date.now() - Date.parse(row.last_seen_at) < 10 * 60_000) };
}
...
function statusForProject(projectId: string) {
  ...
  const work = unfinishedSession(projectId);
  const executor = one('SELECT * FROM operation_executors WHERE current_project_id=? ORDER BY last_seen_at DESC LIMIT 1', projectId);
  const latestOutcome = one('SELECT * FROM operation_outcomes WHERE project_id=? ORDER BY updated_at DESC LIMIT 1', projectId);
...
export const operationCommands = {
  context: async (ctx: CommandContext, input: { projectId?: string; operationId?: string }) => {
  ...
  start: async (ctx: CommandContext, input: OperationStartInput) => {
    ...
    required(requestText, 'Operation request text is required');
    if (input.conversationRef) {
      const continued = one("SELECT id FROM operation_requests WHERE conversation_ref=? AND status IN ('active','awaiting_review','blocked') ORDER BY updated_at DESC LIMIT 1", input.conversationRef);
      if (continued) return { reused: true, operation: operationView(continued.id), reason: 'active_conversation_operation' };
    }
    const projectIds = selectProjects(input);
    for (const projectId of projectIds) assertOperationAllowed(ctx, { projectId, command: 'operation.start', capability: 'operation.start' });
    ...
    const duplicate = one('SELECT id FROM operation_requests WHERE request_key=?', requestKey);
    if (duplicate) return { reused: true, operation: operationView(duplicate.id), reason: 'idempotency_key' };
    ...
    const budget = {
      maxActions: Math.max(1, Math.min(Math.floor(input.budget?.maxActions ?? 12), 100)),
      maxExternalRequests: Math.max(0, Math.min(Math.floor(input.budget?.maxExternalRequests ?? 8), 100)),
      maxCandidateWrites: Math.max(0, Math.min(Math.floor(input.budget?.maxCandidateWrites ?? 50), 1000)),
      maxProjects: projectIds.length,
      maxRuntimeMinutes: Math.max(1, Math.min(Math.floor(input.budget?.maxRuntimeMinutes ?? 30), 1440)),
      maxKnownCost: input.budget?.maxKnownCost ?? 0
    };
    const operationId = randomUUID(); const t = now();
    sqlite.transaction(() => {
      run('INSERT INTO operation_requests(id,request_key,request_text,objective,constraints_json,permissions_json,budget_json,assumptions_json,conversation_ref,status,created_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
        operationId, requestKey, requestText, objective, JSON.stringify(input.constraints ?? {}), JSON.stringify(input.permissions ?? {}), JSON.stringify(budget), JSON.stringify(input.assumptions ?? []), input.conversationRef ?? null, 'active', ctx.actorId ?? ctx.actor, t, t);
    }).immediate();
    ...
    const result = { reused: false, children, operation: operationView(operationId) };
    audit(ctx, 'operation.start', projectIds[0] ?? null, children[0]?.workSessionId ?? null, input, { operationId, projectIds });
    return result;
  },
  ...
  resume: async (ctx: CommandContext, input: { operationId: string; projectId?: string }) => {
    ...
    const children = rows('SELECT * FROM operation_projects WHERE operation_id=?', input.operationId).filter(row => !input.projectId || row.project_id === input.projectId);
    required(children.length, 'Operation has no matching project');
    for (const child of children) {
      ...
      run("UPDATE operation_projects SET status='running',blocker=NULL,updated_at=? WHERE operation_id=? AND project_id=?", now(), input.operationId, child.project_id);
    }
    run("UPDATE operation_requests SET status='active',updated_at=? WHERE id=?", now(), input.operationId);
    emitEvent({ operationId: input.operationId, projectId: input.projectId ?? null, kind: 'operation_resumed', key: `operation:${input.operationId}:resume:${Date.now()}`, payload: { projectId: input.projectId ?? null } });
    return operationView(input.operationId);
  },

  checkpoint: async (ctx: CommandContext, input: { operationId: string; projectId: string; state: 'working' | 'awaiting_review' | 'blocked'; summary: string; nextAction?: string }) => {
    ...
    if (child.work_session_id) {
      ...
      const sessionStatus = input.state === 'working' ? 'running' : input.state;
      run('UPDATE work_sessions SET status=?,summary=?,last_next_action=?,updated_at=? WHERE id=?', sessionStatus, summary.slice(0, 2000), input.nextAction?.trim() || null, now(), child.work_session_id);
    }
    run('UPDATE operation_projects SET status=?,blocker=?,updated_at=? WHERE operation_id=? AND project_id=?', input.state === 'working' ? 'running' : input.state, input.state === 'blocked' ? summary : null, now(), input.operationId, input.projectId);
    if (input.state !== 'working') run('UPDATE operation_requests SET status=?,updated_at=? WHERE id=?', input.state, now(), input.operationId);
    if (input.state === 'blocked') emitEvent({ operationId: input.operationId, projectId: input.projectId, kind: 'major_failure', severity: 'error', key: `operation:${input.operationId}:blocked:${fingerprint(summary)}`, payload: { summary, nextAction: input.nextAction ?? null } });
    return operationView(input.operationId);
  },
  ...
  complete: async (ctx: CommandContext, input: { operationId: string; summary: string }) => {
    ...
    for (const child of children) {
      ...
      run("UPDATE operation_projects SET status='completed',blocker=NULL,updated_at=? WHERE operation_id=? AND project_id=?", now(), input.operationId, child.project_id);
    }
  ...
  startDiscovery: async (ctx: CommandContext, input: { operationId: string; projectId: string; seedKeywords?: string[]; targetUrl?: string; goal: string; language?: string; country?: string; region?: string; excludedTerms?: string[]; maxCandidates?: number; maxExternalRequests?: number }) => {
    ...
    const project = projectExists(input.projectId); const seeds = [...new Set((input.seedKeywords ?? []).map(normalizeText).filter(Boolean))]; const targetUrl = input.targetUrl?.trim() || null;
    required(seeds.length || targetUrl, 'At least one seed keyword or target URL is required');
    const active = one("SELECT * FROM discovery_jobs WHERE project_id=? AND status IN ('waiting_for_agent','running','awaiting_review','blocked') ORDER BY updated_at DESC LIMIT 1", input.projectId);
    if (active) return { reused: true, jobId: active.id, status: active.status, taskId: active.task_id };
    const jobId = randomUUID(); const taskId = randomUUID(); const t = now();
    const maxCandidates = Math.max(1, Math.min(Math.floor(input.maxCandidates ?? 50), 500));
    const maxExternal = Math.max(1, Math.min(Math.floor(input.maxExternalRequests ?? 8), 50));
    sqlite.transaction(() => {
    ...
    emitEvent({ operationId: input.operationId, projectId: input.projectId, kind: 'discovery_queued', key: `operation:${input.operationId}:discovery:${jobId}`, payload: { jobId, goal: input.goal } });
    return { reused: false, jobId, taskId, status: 'waiting_for_agent' };
  },
  ...
  triageCandidates: async (ctx: CommandContext, input: { operationId: string; projectId: string; jobId: string; candidateIds: string[]; status: 'shortlisted' | 'hold' | 'rejected' | 'research_more'; reason: string }) => {
    assertOperationAllowed(ctx, { projectId: input.projectId, command: 'operation.candidate_triage', capability: 'candidate.triage' });
    const child = one('SELECT 1 FROM operation_projects WHERE operation_id=? AND project_id=?', input.operationId, input.projectId); required(child, 'Project is not part of this operation');
    ...
    sqlite.transaction(() => {
      for (const candidateId of ids) {
        const result = run('UPDATE discovery_candidates SET status=?,updated_at=? WHERE id=? AND job_id=? AND project_id=?', input.status, t, candidateId, input.jobId, input.projectId);
        if (result.changes) {
          changed += 1;
          run('INSERT INTO decisions(id,project_id,actor,action,target_type,target_id,verdict,reason,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)', randomUUID(), input.projectId, ctx.actor, 'operation.candidate_triage', 'discovery_candidate', candidateId, input.status, normalizeText(input.reason).slice(0, 1000), JSON.stringify({ operationId: input.operationId, jobId: input.jobId }), t);
        }
      }
    }).immediate();
    return { changed, status: input.status };
  },

  prepareBlogHandoff: async (ctx: CommandContext, input: { operationId: string; projectId: string; pageId: string }) => {
    assertOperationAllowed(ctx, { projectId: input.projectId, command: 'blog.export', capability: 'blog.transport' });
    const child = one('SELECT 1 FROM operation_projects WHERE operation_id=? AND project_id=?', input.operationId, input.projectId); required(child, 'Project is not part of this operation');
    const payload = await blogCommands.export({ ...ctx, projectId: input.projectId }, { projectId: input.projectId, pageId: input.pageId });
    emitEvent({ operationId: input.operationId, projectId: input.projectId, kind: 'handoff_ready', key: `operation:${input.operationId}:handoff:${payload.handoff_id}`, payload: { handoffId: payload.handoff_id, pageId: input.pageId, publicationAuthorized: false } });
    return { handoff: payload, transport: 'direct_command_payload', publicationAuthorized: false };
  },
  ...
  recordOutcome: async (ctx: CommandContext, input: { operationId?: string; projectId: string; targetUrl?: string; handoffId?: string; hypothesis?: string; implementedAt?: string; publishedAt?: string; evaluationDueAt?: string; status: 'pending' | 'improved' | 'regressed' | 'inconclusive' | 'unmeasurable'; metrics?: Record<string, unknown>; attributionNotes?: string; nextAction?: string }) => {
    assertOperationAllowed(ctx, { projectId: input.projectId, command: 'operation.outcome', capability: 'outcome.record', allowWhilePaused: true });
    if (input.operationId) required(one('SELECT 1 FROM operation_projects WHERE operation_id=? AND project_id=?', input.operationId, input.projectId), 'Project is not part of this operation');
    const existing = input.handoffId ? one('SELECT * FROM operation_outcomes WHERE project_id=? AND handoff_id=? ORDER BY updated_at DESC LIMIT 1', input.projectId, input.handoffId) : null;
    const id = existing?.id ?? randomUUID(); const t = now();
    if (existing) run('UPDATE operation_outcomes SET operation_id=?,target_url=?,hypothesis=?,implemented_at=?,published_at=?,evaluation_due_at=?,outcome_status=?,metrics_json=?,attribution_notes=?,next_action=?,updated_at=? WHERE id=?',
    ...
    else run('INSERT INTO operation_outcomes(id,operation_id,project_id,target_url,handoff_id,hypothesis,implemented_at,published_at,evaluation_due_at,outcome_status,metrics_json,attribution_notes,next_action,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      id, input.operationId ?? null, input.projectId, input.targetUrl ?? null, input.handoffId ?? null, input.hypothesis ?? null, input.implementedAt ?? null, input.publishedAt ?? null, input.evaluationDueAt ?? null, input.status, JSON.stringify(input.metrics ?? {}), input.attributionNotes ?? null, input.nextAction ?? null, t, t);
    if (['improved','regressed','unmeasurable'].includes(input.status)) emitEvent({ operationId: input.operationId ?? null, projectId: input.projectId, kind: 'meaningful_outcome', severity: input.status === 'regressed' ? 'warning' : 'info', key: `outcome:${id}:${input.status}`, payload: { outcomeId: id, status: input.status, nextAction: input.nextAction ?? null } });
    return outcomeView(one('SELECT * FROM operation_outcomes WHERE id=?', id));
  },

  listOutcomes: async (_ctx: CommandContext, input: { projectId?: string; status?: string; limit?: number }) => {
    const limit = Math.max(1, Math.min(Math.floor(input.limit ?? 50), 200));
    if (input.projectId && input.status) return rows('SELECT * FROM operation_outcomes WHERE project_id=? AND outcome_status=? ORDER BY updated_at DESC LIMIT ?', input.projectId, input.status, limit).map(outcomeView);
    if (input.projectId) return rows('SELECT * FROM operation_outcomes WHERE project_id=? ORDER BY updated_at DESC LIMIT ?', input.projectId, limit).map(outcomeView);
    if (input.status) return rows('SELECT * FROM operation_outcomes WHERE outcome_status=? ORDER BY updated_at DESC LIMIT ?', input.status, limit).map(outcomeView);
    return rows('SELECT * FROM operation_outcomes ORDER BY updated_at DESC LIMIT ?', limit).map(outcomeView);
  },
  ...
  delegationGrant: async (ctx: CommandContext, input: { projectId: string; capability: string; limits?: Record<string, unknown>; expiresAt?: string }) => {
    assertHuman(ctx, 'Granting an operation delegation'); projectExists(input.projectId);
    if (input.expiresAt && Date.parse(input.expiresAt) <= Date.now()) throw new Error('Delegation expiration must be in the future');
    const limits = input.limits ?? {}; const t = now(); const id = randomUUID();
    sqlite.transaction(() => {
      run("UPDATE operation_delegations SET status='revoked',revoked_at=? WHERE project_id=? AND capability=? AND status='active'", t, input.projectId, input.capability);
      run('INSERT INTO operation_delegations(id,project_id,capability,status,limits_json,version_hash,approved_by,approved_at,expires_at,revoked_at) VALUES(?,?,?,?,?,?,?,?,?,?)', id, input.projectId, input.capability, 'active', JSON.stringify(limits), fingerprint({ capability: input.capability, limits, expiresAt: input.expiresAt ?? null }), ctx.actorId ?? 'human', t, input.expiresAt ?? null, null);
    }).immediate();
    return listDelegations(input.projectId).find(item => item.id === id);
  },
  delegationRevoke: async (ctx: CommandContext, input: { projectId: string; delegationId: string; reason?: string }) => {
    assertHuman(ctx, 'Revoking an operation delegation'); projectExists(input.projectId);
    const t = now(); const result = run("UPDATE operation_delegations SET status='revoked',revoked_at=? WHERE id=? AND project_id=? AND status='active'", t, input.delegationId, input.projectId);
    if (!result.changes) throw new Error('Active delegation not found');
    if (input.reason) emitEvent({ projectId: input.projectId, kind: 'delegation_revoked', key: `delegation:${input.delegationId}:revoked`, payload: { reason: normalizeText(input.reason) } });
    return { id: input.delegationId, status: 'revoked', revokedAt: t };
  },

  executorRegister: async (ctx: CommandContext, input: { executorId?: string; capabilities?: string[]; leaseSeconds?: number }) => {
    if (ctx.actor !== 'agent' && ctx.actor !== 'system') throw new Error('Executor registration requires an agent or system actor');
  ...
  executorHeartbeat: async (ctx: CommandContext, input: { executorId?: string; generation: number; leaseSeconds?: number }) => {
  ...
  executorClaim: async (ctx: CommandContext, input: { executorId?: string; generation: number; operationId: string; projectId: string; leaseSeconds?: number }) => {
    assertOperationAllowed(ctx, { projectId: input.projectId, command: 'operation.executor_claim', capability: 'operation.start' });
    const executorId = input.executorId?.trim() || ctx.actorId?.trim(); required(executorId, 'Executor ID is required');
    required(one('SELECT 1 FROM operation_projects WHERE operation_id=? AND project_id=?', input.operationId, input.projectId), 'Operation project not found');
    const t = now(); const expires = leaseUntil(input.leaseSeconds);
    const result = run(`UPDATE operation_executors SET current_operation_id=?,current_project_id=?,status='busy',last_seen_at=?,lease_expires_at=?,updated_at=?
      ...
      input.operationId, input.projectId, t, expires, t, executorId, input.generation, t, input.operationId, input.projectId);
    if (!result.changes) throw new Error('Executor cannot claim this operation; ownership or generation changed');
    run("UPDATE operation_projects SET status='running',blocker=NULL,updated_at=? WHERE operation_id=? AND project_id=?", t, input.operationId, input.projectId);
    return executorView(one('SELECT * FROM operation_executors WHERE id=?', executorId));
  },
  executorRelease: async (ctx: CommandContext, input: { executorId?: string; generation: number; operationId: string; projectId: string }) => {
    if (ctx.actor !== 'agent' && ctx.actor !== 'system') throw new Error('Executor release requires an agent or system actor');
    const executorId = input.executorId?.trim() || ctx.actorId?.trim(); required(executorId, 'Executor ID is required');
    const result = run("UPDATE operation_executors SET current_operation_id=NULL,current_project_id=NULL,status='online',last_seen_at=?,lease_expires_at=NULL,updated_at=? WHERE id=? AND generation=? AND current_operation_id=? AND current_project_id=?", now(), now(), executorId, input.generation, input.operationId, input.projectId);
    if (!result.changes) throw new Error('Executor release rejected because ownership changed');
    return executorView(one('SELECT * FROM operation_executors WHERE id=?', executorId));
  },

  acknowledgeEvent: async (ctx: CommandContext, input: { eventId: string }) => {
    assertHuman(ctx, 'Acknowledging an operation event');
    const result = run('UPDATE operation_events SET acknowledged_at=? WHERE id=?', now(), input.eventId);
    if (!result.changes) throw new Error('Operation event not found');
    return { id: input.eventId, acknowledgedAt: now() };
  },

  remoteReadiness: async (_ctx: CommandContext) => ({
    generatedAt: now(),
    database: { path: getDatabase().path, remoteSafe: process.env.KEYWORDS_REMOTE_DB_READY === '1', note: process.env.KEYWORDS_REMOTE_DB_READY === '1' ? 'Remote persistence explicitly marked ready.' : 'Local SQLite is the default; do not run multiple remote writers without a managed persistence decision.' },
    api: { bindHost: process.env.KEYWORDS_API_HOST ?? '127.0.0.1', humanTokenConfigured: Boolean(process.env.KEYWORDS_API_HUMAN_TOKEN), agentTokenConfigured: Boolean(process.env.KEYWORDS_API_AGENT_TOKEN), allowedOriginsConfigured: Boolean(process.env.KEYWORDS_ALLOWED_ORIGINS) },
    executor: { persistentRequested: process.env.KEYWORDS_EXECUTOR_PERSISTENT === '1', maxRetryAttempts: Math.max(1, Number(process.env.KEYWORDS_EXECUTOR_MAX_RETRIES ?? 5)) },
    recommendation: process.env.KEYWORDS_REMOTE_DB_READY === '1' && process.env.KEYWORDS_API_AGENT_TOKEN ? 'remote_host_eligible' : 'keep_local_executor'
  })
};
