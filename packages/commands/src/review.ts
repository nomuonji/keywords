...
function defaultOptions(targetType: string) {
  if (targetType === 'page') return ['approved', 'needs_edit', 'rejected'];
  if (targetType === 'policy') return ['active', 'rejected'];
...
async function requireTarget(projectId: string, targetType: string, targetId?: string) {
  ...
  if (targetType === 'page') {
    ...
    if (!page) throw new Error('Review target page not found');
    if (page.status !== 'proposed') throw new Error('Only proposed pages should be sent for review');
  }
  if (targetType === 'policy') {
    ...
    if (!policy) throw new Error('Review target policy not found');
    if (policy.status !== 'candidate') throw new Error('Only candidate policies should be sent for review');
  }
}

function normalize(row: typeof schema.reviewRequests.$inferSelect) {
  return { ...row, options: parseOptions(row.optionsJson), optionsJson: undefined };
}
...
export const reviewCommands = {
  list: async (ctx: CommandContext, input: { projectId: string; status?: string; sessionId?: string; limit?: number }) => withRun(projectCtx(ctx, input.projectId, input.sessionId), 'review.list', input, async () => {
    ...
    const conditions = [eq(schema.reviewRequests.projectId, input.projectId)];
    if (input.status) conditions.push(eq(schema.reviewRequests.status, input.status));
    if (input.sessionId) conditions.push(eq(schema.reviewRequests.workSessionId, input.sessionId));
    const rows = await db.select().from(schema.reviewRequests).where(and(...conditions)).orderBy(desc(schema.reviewRequests.createdAt)).limit(limit);
    return rows.map(normalize);
  }),

  request: async (ctx: CommandContext, input: {
    projectId: string;
    sessionId?: string;
    targetType: string;
    targetId?: string;
    ...
    question?: string;
    options?: string[];
  }) => withRun(projectCtx(ctx, input.projectId, input.sessionId), 'review.request', input, async () => {
    const sessionId = input.sessionId ?? ctx.workSessionId;
    if (!sessionId) throw new Error('Review request requires a work session');
    const session = await requireSession(input.projectId, sessionId);
    await requireTarget(input.projectId, input.targetType, input.targetId);
    const title = input.title.trim();
    if (!title) throw new Error('Review request title is required');
    const options = [...new Set((input.options?.length ? input.options : defaultOptions(input.targetType)).map(value => value.trim()).filter(Boolean))].slice(0, 8);
    if (options.length < 2) throw new Error('Review request needs at least two resolution options');
    const t = now();
    const row = {
      id: id(), projectId: input.projectId, workSessionId: session.id, targetType: input.targetType, targetId: input.targetId ?? null,
      title, question: input.question?.trim() || null, optionsJson: JSON.stringify(options), status: 'open', resolution: null, reason: null,
      requestedBy: ctx.actorId ?? ctx.actor, createdAt: t, resolvedAt: null
    };
    await db.insert(schema.reviewRequests).values(row);
    const checkpointSummary = `Human review requested: ${title}`;
  ...
  resolve: async (ctx: CommandContext, input: {
    projectId: string;
    reviewId: string;
    resolution: string;
    reason?: string;
    overrideConflicts?: boolean;
  }) => withRun(projectCtx(ctx, input.projectId), 'review.resolve', input, async () => {
    ...
    if (request.status !== 'open') throw new Error('Review request is already resolved');
    const options = parseOptions(request.optionsJson);
    if (!options.includes(input.resolution)) throw new Error(`Resolution must be one of: ${options.join(', ')}`);

    const targetCtx = projectCtx(ctx, input.projectId, request.workSessionId ?? undefined);
    ...
    if (request.targetType === 'page' && request.targetId) {
      ...
      targetResult = await planningCommands.pageReview(targetCtx, {
        projectId: input.projectId,
        pageId: request.targetId,
        verdict: input.resolution as 'approved' | 'rejected' | 'needs_edit',
        reason: input.reason,
      ...
      targetResult = await policyCommands.review(targetCtx, {
        projectId: input.projectId,
        policyId: request.targetId,
        verdict: input.resolution as 'active' | 'rejected',
        reason: input.reason
      });
    } else {
      const decision = {
        id: id(), projectId: input.projectId, actor: ctx.actor, action: 'review.resolve', targetType: request.targetType,
        targetId: request.targetId, verdict: input.resolution, reason: input.reason?.trim() || null,
        metadataJson: JSON.stringify({ reviewRequestId: request.id, title: request.title }), createdAt: now()
      };
      await db.insert(schema.decisions).values(decision);
      targetResult = { decisionId: decision.id };
    }

    const t = now();
    await db.update(schema.reviewRequests).set({ status: 'resolved', resolution: input.resolution, reason: input.reason?.trim() || null, resolvedAt: t }).where(eq(schema.reviewRequests.id, request.id));
    ...
    const updated = await db.select().from(schema.reviewRequests).where(eq(schema.reviewRequests.id, request.id)).get();
    return { review: updated ? normalize(updated) : null, targetResult };
  })
};
