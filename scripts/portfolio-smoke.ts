...
const operation = await operationCommands.start(human, { requestText: 'Build the scalable article index fixture', projectIds: [project.id], requestKey: 'portfolio-article-index-fixture' });
const operationId = operation.operation.id as string;
...
sqlite.prepare(`INSERT INTO operation_artifacts(id,operation_id,project_id,page_id,article_id,artifact_path,content_sha256,source_ids_json,validator_version,validator_status,build_status,after_hash,manifest_json,generated_at,verified_at,updated_at)
  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run('portfolio-artifact', operationId, project.id, page.id, page.id, 'content/scalable-article-index.md', articleHash, JSON.stringify(['source-1']), 'fixture', 'passed', 'passed', articleHash, '{}', t, t, t);
