...
const evidence = await commands.source.record(agent, { projectId: project.id, type: 'smoke', label: 'Smoke discovery evidence', metadata: { verified: true } });
const started = await discoveryCommands.start(human, { projectId: project.id, seedKeywords: ['seo 自動化'], goal: 'Find decision-ready article opportunities', maxCandidates: 3, maxExternalRequests: 2 });
...
const imported = await discoveryCommands.importCandidates(agentSession, { projectId: project.id, jobId: started.job.id, candidates: [
  { keyword: '求人 seo', demandValue: 999, demandProvider: 'smoke', observedAt: new Date().toISOString(), sourceId: evidence.id },
  { keyword: known.text, demandValue: 80, demandProvider: 'smoke', observedAt: new Date().toISOString(), sourceId: evidence.id },
  { keyword: 'seo 自動化 方法', demandValue: 120, demandProvider: 'smoke', observedAt: new Date().toISOString(), adCompetition: 0.35, sourceId: evidence.id },
  { keyword: 'seo 自動化 ツール', demandValue: 90, demandProvider: 'smoke', observedAt: new Date().toISOString(), sourceId: evidence.id },
...
if (!continuous.learning.rejectReasons.some(x => x.reason.includes('tool-specific')) || !continuous.learning.holdReasons.length || !continuous.learning.providerPerformance.some(x => x.provider === 'smoke')) throw new Error('Continuous discovery learning failed');
...
const recoveryJob = await discoveryCommands.start(human, { projectId: recoveryProject.id, seedKeywords: ['lease recovery'], goal: 'Test executor recovery' });
