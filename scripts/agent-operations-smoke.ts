...
await assert.rejects(
  () => operationCommands.start(agent, { requestText: 'Improve one existing page', projectIds: [projectId], requestKey: 'smoke-operation' }),
  /not delegated/
);
...
const started = await operationCommands.start(agent, {
  requestText: 'Improve one existing page',
  projectIds: [projectId],
  requestKey: 'smoke-operation',
  conversationRef: 'smoke-conversation',
  budget: { maxActions: 8, maxExternalRequests: 2, maxCandidateWrites: 3, maxProjects: 1 }
});
...
const repeated = await operationCommands.start(agent, {
  requestText: 'Improve one existing page', projectIds: [projectId], requestKey: 'smoke-operation', conversationRef: 'smoke-conversation'
});
assert.equal(repeated.reused, true);
assert.equal(repeated.operation.id, operationId);
...
settleOperationBudget(r1?.id, 'succeeded');
...
const discovery = await operationDiscoveryCommands.startAndClaim(agentWork, {
  operationId, projectId, seedKeywords: ['agent seo'], goal: 'smoke discovery', maxCandidates: 3, maxExternalRequests: 2
});
assert.equal(discovery.status, 'running');
assert.equal(discovery.workSessionId, child.workSessionId);
const discoveryAgain = await operationDiscoveryCommands.startAndClaim(agentWork, {
  operationId, projectId, seedKeywords: ['agent seo'], goal: 'smoke discovery', maxCandidates: 3, maxExternalRequests: 2
});
...
await assert.rejects(
  () => operationCommands.triageCandidates(agentWork, { operationId, projectId, jobId: discovery.jobId, candidateIds: [candidateId], status: 'hold', reason: 'needs more evidence' }),
  /not delegated/
);
...
await assert.rejects(
  () => operationCommands.executorHeartbeat(agent, { executorId: 'smoke-executor', generation: registered.generation }),
  /generation changed/
);
...
assert.equal(scopeA.targetOrigin, 'https://a.example.com');
assert.equal(scopeB.targetOrigin, 'https://b.example.com');
assert.notDeepEqual(scopeA.filters, scopeB.filters);
...
assert.equal(pairs[0].latest.startDate, '2026-08-08');
